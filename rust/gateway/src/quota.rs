...
///   - Background tokio task flushes cache → PostgreSQL every 10 seconds
...
impl QuotaStore {
    /// Initialize the store from an existing PostgreSQL pool.
    /// The `token_usage` table is expected to already exist (created by migrations).
    pub async fn new(daily_quota: u64, pool: PgPool) -> Result<Arc<Self>> {
        info!("quota store initialized (PostgreSQL)");
    ...
    /// Load today's usage rows from PostgreSQL into the in-memory cache.
    async fn load_today(&self) -> Result<()> {
        let today = today();
        let rows: Vec<(String, i64, i64)> = sqlx::query_as(
            "SELECT key_hash, prompt_tokens, completion_tokens
             FROM token_usage WHERE date = $1::date",
        )
        .bind(&today)
        .fetch_all(&self.pool)
        .await?;
    ...
    /// Record token usage. Atomic fetch_add on the in-memory cache; PostgreSQL is
    ...
    /// Upsert a single row into PostgreSQL. Called from the background flush.
    pub async fn record_request_db(
        ...
        sqlx::query(
            "INSERT INTO token_usage (key_hash, date, prompt_tokens, completion_tokens, request_count)
    ...
    pub async fn get_usage(&self, api_key: &str) -> Result<UsageInfo> {
        ...
        let row: Option<(i64, i64, i64)> = sqlx::query_as(
            "SELECT prompt_tokens, completion_tokens, request_count
             FROM token_usage WHERE key_hash = $1 AND date = $2::date",
        )
        .bind(&key_hex)
        .bind(&today_str)
    ...
    /// Flush in-memory cache to PostgreSQL and evict stale date entries.
    /// Called by the background task every 10 seconds.
